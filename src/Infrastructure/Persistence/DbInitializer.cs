...
public static class DbInitializer
{
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        // --- 2. ТИПИ ТАРИ ---
        var glass = ContainerType.New("Скляна пляшка", null, null);
        var plastic = ContainerType.New("Пластикова каністра", null, null);
        var metal = ContainerType.New("Металева бочка", null, null);
        var silo = ContainerType.New("Елеваторний силос", null, null);
        ...
        var cEmpty = Container.New("EMPTY-01", "Нова бочка", 200m, metal.Id, null, userId);
        context.Containers.Add(cEmpty);

        // Сценарій 2: Заповнена тара (свіжа)
        var cFull = Container.New("MILK-01", "Бідон 20л", 20m, plastic.Id, null, userId);
        ...
        var cExpired = Container.New("YOG-EXP", "Контейнер йогурту", 5m, plastic.Id, null, userId);
        ...
        var cWasFull = Container.New("CLEAN-01", "Каністра після хлору", 10m, plastic.Id, null, userId);
        ...
        var cHistory = Container.New("HIST-01", "Багаторазова пляшка", 1m, glass.Id, null, userId);
        ...
        var cSilo = Container.New("SILO-01", "Силос №1", 10000m, silo.Id, null, userId);
